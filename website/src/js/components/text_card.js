...
export function TextCard(props) {
  ...
  function makeURL(courseName, courseCode) {
    ...
    if (courseCode.match(/\([a-z]\)/)) {
      url = url + courseCode.slice(0,-3) + '-' + courseCode.slice(-2,-1).toUpperCase() + '?cb_cycle=bama_cyclemaster';
    }
    else {
        ...
        <CardContent>
          ...
          >
            {(props.isURL) ?
                <Link href="#"  onClick={() => {
                  if (props.isURL) {
                    const win = window.open(makeURL(props.text, props.courseCode), '_blank');
                    win.focus();}}
                }>
                  {props.text}
                </Link>
